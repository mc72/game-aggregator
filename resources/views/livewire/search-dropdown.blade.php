<div class="relative">
    <input wire:model.debounce.300ms="search" type="text" class="w-64 px-3 py-1 pl-8 text-sm bg-gray-800 rounded-full focus:outline-none focus:shadow-outline" placeholder="Search...">
    ...
    @if(strlen($search )>= 2)
    <div class="absolute z-50 w-64 mt-2 text-sm bg-gray-800 rounded">
        @if(count($searchResults) > 0)
        <ul>
            @foreach($searchResults as $game)
            <li class="border-b border-gray-700">
                <a href="{{ route('games.show', $game['slug']) }}" class="flex items-center block px-3 py-3 transition duration-150 ease-in-out hover:bg-gray-700">
                    @if(isset($game['cover']))
